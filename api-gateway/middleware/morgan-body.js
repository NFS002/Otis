...
module.exports = function morganBody (app, options) {
	// default options
	options = options || {}
	var maxBodyLength = Object.hasOwnProperty.call(options, "maxBodyLength") ? options.maxBodyLength : 1000
	var logReqDateTime = Object.hasOwnProperty.call(options, "logReqDateTime") ? options.logReqDateTime : true
	var logAllReqHeader = Object.hasOwnProperty.call(options, "logAllReqHeader") ? options.logAllReqHeader : false
	var logAllResHeader = Object.hasOwnProperty.call(options, "logAllResHeader") ? options.logAllResHeader : false
	var logReqHeaderList = Object.hasOwnProperty.call(options, "logReqHeaderList") ? options.logReqHeaderList : null
	var logResHeaderList = Object.hasOwnProperty.call(options, "logResHeaderList") ? options.logResHeaderList : null
	var logReqUserAgent = Object.hasOwnProperty.call(options, "logReqUserAgent") ? options.logReqUserAgent : true
	var logRequestBody = Object.hasOwnProperty.call(options, "logRequestBody") ? options.logRequestBody : true
	var logResponseBody = Object.hasOwnProperty.call(options, "logResponseBody") ? options.logResponseBody : true
	var timezone = Object.hasOwnProperty.call(options, "timezone") ? options.timezone || "local" : "local"
	var noColors = Object.hasOwnProperty.call(options, "noColors") ? options.noColors : false
	var prettify = Object.hasOwnProperty.call(options, "prettify") ? options.prettify : true

	var theme
	if (noColors) {
		if (Object.hasOwnProperty.call(options, "theme"))
			console.log(`\n\nWARNING: provided theme when 'noColor' option was set to true, theme provided: ${theme}, theme will be ignored\n\n`)

		theme = "noColorsTheme"
	} else {
		theme = Object.hasOwnProperty.call(options, "theme") ? options.theme : "defaultTheme"
	}
	...
	var morganOptions = {}
	if (Object.hasOwnProperty.call(options, "buffer"))
	...
	if (Object.hasOwnProperty.call(options, "immediate"))
	...
	if (Object.hasOwnProperty.call(options, "stream"))
		morganOptions.stream = options.stream

	if (Object.hasOwnProperty.call(options, "skip"))
		morganOptions.skip = options.skip

	morganOptions.prettify = prettify


	if (logReqDateTime) {
		var dateTimeFormat = Object.hasOwnProperty.call(options, "dateTimeFormat") ? options.dateTimeFormat || "" : ""
		if (typeof dateTimeFormat !== "string") { throw new Error(`morgan-body was passed a non-string value for "dateTimeFormat" option, value passed was: ${dateTimeFormat}`) } else {
	...
	morgan.format(morganReqFormatName, function developmentFormatLine (tokens, req, res) {
		...
		if (!fn) {
			...
			if (logAllReqHeader) { formatString += " headers[:request-headers]" } else
	...
	if (logRequestBody || logResponseBody) {
		...
		if (logResponseBody) {
			...
			app.use(morgan(logBodyGen("Response", (req, res) => res.__morgan_body_response || res.locals.body), morganOptions))
		}
	}
	...
	morgan.format(morganResFormatName, function developmentFormatLine (tokens, req, res) {
		...
		if (!fn) {
			...
			if (logAllResHeader) { formatString += " headers[:response-headers]" + defaultColor } else
...
function isoDate (dateObj) {
	var currentDate = pad2(dateObj.date())
	var currentMonth = pad2(dateObj.month() + 1)
	var currentYear = pad2(dateObj.year())
	var currentHrs = pad2(dateObj.hours())
	var currentMins = pad2(dateObj.minutes())
	var currentSecs = pad2(dateObj.seconds())
	var currentMillisecs = pad3(dateObj.milliseconds())
	var timezoneOffset = dateObj.utcOffset()

	if (timezoneOffset === 0) {
		timezoneOffset = "Z"
	} else {
		var offsetHrs = pad2(parseInt(Math.abs(timezoneOffset / 60)))
		var offsetMin = pad2(Math.abs(timezoneOffset % 60))
		var sign = timezoneOffset > 0 ? "+" : "-"
		timezoneOffset = sign + offsetHrs + ":" + offsetMin
	}

	return currentYear + "-" + currentMonth + "-" + currentDate + "T" + currentHrs +
	    ":" + currentMins + ":" + currentSecs + "." + currentMillisecs + timezoneOffset
}
