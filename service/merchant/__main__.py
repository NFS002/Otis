...
from service.lib.psql import get_engine
...
def serve():
    ...
    rdb_engine_conf = get_value(SERVICE_CONFIG, 'rdb_engine', default={})
    db_engine = get_engine(**rdb_engine_conf)
    merchant_service_pb2_grpc.add_MerchantServiceServicer_to_server(MerchantService(db_engine=db_engine), server)
    server.start()
